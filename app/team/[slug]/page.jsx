...
export default async function TeamPage({ params }) {
  ...
  const pokemonWithDefenses = await Promise.all(
    pokemon.map(async (p) => {
      ...
      const weakTo = Object.entries(combined)
        .filter(([, m]) => m >= 2)
        .sort((a, b) => b[1] - a[1]);
      const resistantTo = Object.entries(combined)
        .filter(([, m]) => m <= 0.5 && m > 0)
        .sort((a, b) => a[1] - b[1]);
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          Equipo de{" "}
          <span className="font-medium text-gray-700">{team.user.name}</span>
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {weaknesses.map(([type, count]) => (
            ...
            >
              ...
              <span
                className={`text-sm font-bold ${count > 0 ? "text-red-500" : "text-green-600"}`}
              >
                {count > 0
                  ? `${count} ${count === 1 ? "débil" : "débiles"}`
                  : `${Math.abs(count)} ${Math.abs(count) === 1 ? "resiste" : "resisten"}`}
              </span>
            </div>
          ))}
        </div>
